  ...
  while ($rowCutoff = mysqli_fetch_array($resultCutoff)) :
    ...
    if ($gross != 0) { // para mag-print lang is yung may mga gross sa cut-off date
      ...
      if ($cutoff_id != $payslip_cutoffID && $empID != $payslip_empID) {
        ...
        (employee_id, cutoff_id, position, tax_status, basic_pay, total_hour, ot, cash_advance, sss, philhealth, hdmif, tax, material_cost, ytd_sss, ytd_philhealth, ytd_hdmif, ytd_grossincome, total_compensation, total_deduc, netpay) 
        VALUES ('$empID', '$cutoff_id', '$position', '$tax_stat', '$gross_payslip', '$total_hour', '$total_ot', '$total_cashad', '$sss_payslip', '$philhealth_payslip', '$pagibig_payslip', '$tax_payslip', '$material_loss', '$sss', '$philhealth', '$pagibig', '$gross', '$compensation_total', '$deduction_total', '$netpay')";
